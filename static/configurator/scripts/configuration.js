...
Configuration.prototype.isComplete = function () {
  const doneFeatures = this.selectedFeatures
    .concat(this.deselectedFeatures)
    .concat(this.getDeactivatedFeatures())
    .concat(this.getActivatedFeatures());
  const allDoneFeatures = this.model.features.reduce(function (acc, feature) {
    return acc && !!doneFeatures.find(featureFinder(feature.name));
  }, true);
  return this.isValid() && allDoneFeatures;
};
...
Configuration.prototype.getDeactivatedFeatures = function () {
  ...
  if (self._deactivatedFeatures === undefined)
    self._deactivatedFeatures = self.model.features.filter(function (feature) {
      if (
        self._activatedFeatures &&
        self._activatedFeatures.find(featureFinder(feature.name))
      )
...
Configuration.prototype.getActivatedFeatures = function () {
  ...
  if (self._activatedFeatures === undefined)
    self._activatedFeatures = self.model.features.filter(function (feature) {
      if (
        self._deactivatedFeatures &&
        self._deactivatedFeatures.find(featureFinder(feature.name))
      )
        return false;
...
Configuration.prototype.isManual = function (feature) {
  return (
    this.getSelectedFeature(feature.name) ||
    this.getDeselectedFeature(feature.name)
  );
};
...
Configuration.prototype.serialize = function () {
  ...
  self.model.features.forEach(function (feature) {
    ...
    var manual = self.getSelectedFeature(feature.name)
      ? "selected"
      : self.getDeselectedFeature(feature.name)
      ? "unselected"
      : "undefined";
    setAttribute(
      node,
      "automatic",
      manual === "undefined" && self.isActivated(feature)
        ? "selected"
        : manual === "undefined" && self.isDeactivated(feature)
        ? "unselected"
        : "undefined"
    );
...
Configuration.prototype.serializeIncomplete = function () {
  var self = this;
  //if (!self.isComplete()) throw "configuration is not complete";
  var xml = document.implementation.createDocument(null, "configuration");
  function setAttribute(node, key, value) {
  ...
  //console.log("current configuration : " + xml);
  return new XMLSerializer().serializeToString(xml);
};
